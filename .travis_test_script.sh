...
cd ../server
ADDONS=$(python -c "import os;print ','.join([f for f in os.listdir('../openerp-addons-ci') if 'l10n_' not in f and '.' not in f and 'ldap' not in f and 'google' not in f and 'document_ftp' not in f])")
